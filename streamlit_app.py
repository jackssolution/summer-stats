...
def show_pitching(player):
    ...
    m[10].metric("K/9",     f"{pt.get('K_per9', 0.0):.1f}")
    m[11].metric("BB/9",    f"{pt.get('BB_per9', 0.0):.1f}")
    m[12].metric("STR%",    f"{pt.get('strike_pct', 0.0):.1f}%")
    m[13].metric("PITCHES", pt.get('total_pitches', 0))
    if player['pitching']:
